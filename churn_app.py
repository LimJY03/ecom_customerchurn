...
if side_menu =='User Manual':
    ...
    st.write('3. Upload your csv file with all the required features/data, it will takes some time to clean and process your file')
    ...
    datadict = pd.read_csv('ECommerce DataDict.csv')
    st.write(datadict)
...
if side_menu=='Analysis and Prediction':
    ...
    if add_selectbox == 'File':
        file_upload = st.file_uploader('Upload csv file for churn prediction', type=['csv'])
        st.write('Please make sure your file have these columns - CustomerID, Tenure, PreferredLoginDevice, CityTier, WarehouseToHome, PreferredPaymentMode, Gender, HourSpendOnApp, NumberOfDeviceRegistered, PreferedOrderCat, SatisfactionScore, MartialStatus, NumberOfAddress, Complain, OrderAmountHikeFromlastYear, CouponUsed, OrderCount, DaySinceLastOrder, CashbackAmount')
        st.write('Can read the data dictionary in **User Manual Page** to understand what each column means')
        ...
        if file_upload is not None:
            ...
            X = df2[df2['CustomerID'] == selected_id].iloc[0, 1:20].values
